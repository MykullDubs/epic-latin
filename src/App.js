  ...
  return (
    <div className="px-6 mt-4 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <section className="space-y-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        <h3 className="font-bold text-slate-800 flex items-center gap-2"><FileText size={18} className="text-indigo-600"/> Basics</h3>
        <input className="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm font-bold" placeholder="Lesson Title" value={data.title} onChange={e => setData({...data, title: e.target.value})} />
        <input className="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm" placeholder="Subtitle" value={data.subtitle} onChange={e => setData({...data, subtitle: e.target.value})} />
        <textarea className="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm" rows={2} placeholder="Description" value={data.description} onChange={e => setData({...data, description: e.target.value})} />
        <input className="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm" placeholder="Vocab list (comma separated)" value={data.vocab} onChange={e => setData({...data, vocab: e.target.value})} />
      </section>
      <section className="space-y-4">
        <h3 className="font-bold text-slate-800 flex items-center gap-2 px-1"><MessageSquare size={18} className="text-indigo-600"/> Dialogue</h3>
        {data.dialogue.map((line, i) => (
          <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group">
            <button onClick={() => removeLine(i)} className="absolute top-2 right-2 text-slate-300 hover:text-red-500"><Trash2 size={16} /></button>
            <div className="flex gap-2 mb-2">
              <input className="flex-1 p-2 bg-slate-50 rounded border border-slate-100 text-xs font-bold" placeholder="Speaker" value={line.speaker} onChange={e => updateDialogue(i, 'speaker', e.target.value)} />
              <select className="p-2 bg-slate-50 rounded border border-slate-100 text-xs" value={line.side} onChange={e => updateDialogue(i, 'side', e.target.value)}>
                <option value="left">Left (You)</option>
                <option value="right">Right (Other)</option>
              </select>
            </div>
            <input className="w-full mb-2 p-2 bg-slate-50 rounded border border-slate-100 text-sm" placeholder="Latin text" value={line.text} onChange={e => updateDialogue(i, 'text', e.target.value)} />
            <input className="w-full p-2 bg-slate-50 rounded border border-slate-100 text-xs italic text-slate-500" placeholder="English translation" value={line.translation} onChange={e => updateDialogue(i, 'translation', e.target.value)} />
          </div>
        ))}
        <button onClick={addLine} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 font-bold flex items-center justify-center gap-2 hover:bg-slate-50 hover:border-indigo-200 hover:text-indigo-500 transition-all"><Plus size={18} /> Add Line</button>
      </section>
      <section className="space-y-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        <h3 className="font-bold text-slate-800 flex items-center gap-2"><Brain size={18} className="text-indigo-600"/> Quiz</h3>
        <input className="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm font-bold" placeholder="Question text" value={data.quiz.question} onChange={e => setData({...data, quiz: {...data.quiz, question: e.target.value}})} />
        <div className="space-y-2">
          {data.quiz.options.map((opt, i) => (
            <div key={opt.id} className="flex gap-2 items-center">
              <input type="radio" name="correct" checked={data.quiz.correctId === opt.id} onChange={() => setData({...data, quiz: {...data.quiz, correctId: opt.id}})} />
              <input className="flex-1 p-2 bg-slate-50 rounded border border-slate-100 text-sm" placeholder={`Option ${opt.id.toUpperCase()}`} value={opt.text} onChange={e => updateOption(i, e.target.value)} />
            </div>
          ))}
        </div>
      </section>
